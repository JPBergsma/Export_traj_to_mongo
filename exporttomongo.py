...
def load_trajectory_data(
    ...
    el = struct.composition.reduced_composition.alphabetical_formula.split()
    for i in range(len(el)):
        if re.match("[^0-9]+1$", el[i]):
            length = len(el[i])
            el[i] = el[i][: length - 1]
    chemical_formula_reduced = "".join(el)
    # chemical_formula_reduced = (
    ...
    trajectories_coll.collection.update_one(
        {"_id": mongoid}, {"$set": {"_hdf5file_path": hdf5path, "id": id}}
    )
    if (
        type == "trajectories"
        ...
        if n_frames*nsites*3*4 > 16*1024: # If the trajectory is larger than 16 kb store it in hdf5 file
            with h5py.File(
                hdf5path, "w"
                ...
                #    if entry["_exmpl_time"]["_storage_location"] == "file":
            ...
            if "F" in elements: # To cover all the different cases for testing I encode the information about the trajectory in a different ways here.                for i in range(n_frames):
                import random
                frames = []
                ...
                trajectories_coll.collection.update_one(
                    {"_id": mongoid}, {"$set": {"cartesian_site_positions.frames": frames, "cartesian_site_positions.frame_serialization_format" : "explicit_custom_sparse", "cartesian_site_positions.nvalues": len(frames)}}
                )
                trajectories_coll.collection.update_one(
                    {"_id": mongoid}, {"$set": {"available_properties.cartesian_site_positions.frame_serialization_format": "explicit_custom_sparse",
                ...
                for i in range(0, n_frames, 2):
                    positions.append(traj.trajectory[i].positions.tolist())
                trajectories_coll.collection.update_one(
                    {"_id": mongoid}, {"$set": {"cartesian_site_positions.step_size_sparse": 2, "cartesian_site_positions.offset_sparse": 0, "cartesian_site_positions.frame_serialization_format": "explicit_regular_sparse", "available_properties.cartesian_site_positions.nvalues": len(positions)}}
                )
                trajectories_coll.collection.update_one(
                    {"_id": mongoid}, {"$set": {"available_properties.cartesian_site_positions.frame_serialization_format": "explicit_regular_sparse",
                ...
                for i in range(n_frames):
                    positions.append(traj.trajectory[i].positions.tolist())
            trajectories_coll.collection.update_one(
                {"_id": mongoid}, {"$set": {"cartesian_site_positions._storage_location": "mongo", "cartesian_site_positions.values": positions}}
            )
